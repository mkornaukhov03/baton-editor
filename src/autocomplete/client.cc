...
void Client::SetConnections() {
    ...
    connect(process_.get(), SIGNAL(readyReadStandardOutput()), this, SLOT(OnClientReadyReadStdout()));
    connect(process_.get(), SIGNAL(readyReadStandardError()), this, SLOT(OnClientReadyReadStderr()));
...
Client::RequestType Client::RangeFormatting(DocumentUri uri, Range range) {
    return SendRequest("textDocument/RangeFormatting", DocumentRangeFormattingParams{uri, range});
}
...
Client::RequestType Client::Completion(DocumentUri uri, Position position,
    ...
    return SendRequest("textDocument/completion", CompletionParams{uri, position, context});
}
...
Client::RequestType Client::SendRequest(std::string method,
                                        json json_doc) {
...
void Client::RequestImpl(std::string method, json params,
                         RequestType type) {
